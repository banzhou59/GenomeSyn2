...
# 替代 grep -Ff 功能，用纯 Perl 实现
if (!-e $gff_file_gai) {
    # 读取 genome_id 文件，将每一行存入哈希
    open(my $GENOME, "<", $genome_id) or die "Cannot open $genome_id: $!\n";
    my %ids;
    while (<$GENOME>) {
        chomp;
		my @arr0 = split(/\s+/, $_);
    ...
    open(my $OUT, ">", $gff_file_gai) or die "Cannot open $gff_file_gai: $!\n";

    while (<$GFF>) {
        chomp;
        my @cols = split(/\t/, $_);
        next unless @cols;  # skip empty lines
        if (exists $ids{$cols[0]}) {  # 第 1 列匹配 genome_id
            print $OUT $_, "\n";
        }
    }
    ...
    print "⚠ $gff_file_gai already exists, skipping extraction.\n";
}
if(0){
	if (!-e $gff_file_gai) {
