...
my $genome_pathX = '';
my $gff_pathX = '';
my $type_priority = 'identity';#density,identity,unite
...
my $mode_snp1  = (defined $identity && defined $density);
my $mode_snp2  = (defined $identity);
my $mode_snp3  = (defined $density);
...
elsif ($mode_snp1 && !$has_exclusive_snp) {$valid = 4;}
elsif ($mode_snp2 && !$has_exclusive_snp) {$valid = 5;}
elsif ($mode_snp3 && !$has_exclusive_snp) {$valid = 6;}
...
if($valid == '1') {
	...
	if (defined $type_priority) {
		if($type_priority =~ /^(fa|fasta|fas|fna|tsv|txt)$/i) {
		...
		elsif($type_priority =~ /^(syn|synteny)$/i) {
			# 读取文件并筛选出以数字打头的 .fa 或 .fasta 或.tsv 或.txt 文件
			my @files = grep { /^\d+\.(.+?)\.(?:|bed|tsv|txt)$/i } readdir($dh);
			closedir($dh);
			# 排序文件（按数字编号）
			@files = sort {
				($a =~ /^(\d+)/)[0] <=> ($b =~ /^(\d+)/)[0]
			...
			open(my $out_fh, ">", $output) or die "Cannot open output file '$output': $!";
			# 打印表头
			print $out_fh "#number\tref_file\tquery_file\talign_file\n";
			# 输出每一行内容
			my $count = 0;
			foreach my $file (@files) {
				my $full_path = abs_path(File::Spec->catfile($path, $file));
				...
				$count++;
				my $NA_out = 'NA';
				print $out_fh join("\t", $count, $NA_out, $NA_out, $full_path), "\n";
			}
			close $out_fh;
			# 在终端提示
			print "✅ Output file has been generated: $output\n";
	...
	#绘制染色体块及刻度线
	draw_vcf_chr_block($SVG1, \%contig_len, \%sample_color, \%SNP_density, \%SNP_identity, \%sample_index, $track, $smooth);
	...
	my $countline = 0;
	my (@arr_sample2, @arr_sample);
	my (%contig_len, %sample_color, %sample_index, %SNP_identity, %sample_color0);
	my (%hash_bin1);
	open(my $IN_SNPidentity,  "<", $identity)  or die "Cannot open $identity: $!";
	while (<$IN_SNPidentity>) {
		chomp;
	...
	#绘制染色体块及刻度线
	draw_identity_chr_block($SVG1, \%contig_len, \%sample_color, \%SNP_identity, \%sample_index, $track, $smooth);
	my $svg_file = $output;
	my $svg_pdf = 1;
	if($svg_file =~ /\.pdf$/i){ $svg_file =~ s/\.pdf$/.svg/i; $svg_pdf = 2;}
	if($svg_file =~ /\.png$/i){ $svg_file =~ s/\.png$/.svg/i; $svg_pdf = 3;}
	...
	my $countline = 0;
	my (@arr_sample2, @arr_sample);
	my (%contig_len, %sample_color, %sample_index, %SNP_density, %sample_color0);
	my (%hash_bin1, %hash_bin2);
	open(my $IN_SNPdensity,  "<", $density)  or die "Cannot open $density: $!";
	while (<$IN_SNPdensity>) {
		chomp;
	...
	#绘制染色体块及刻度线
	draw_density_chr_block($SVG1, \%contig_len, \%sample_color, \%SNP_density, \%sample_index, $track, $smooth);
	#my ($SVG1, $contig_len_ref, $sample_color_ref, $SNP_density_ref, $SNP_identity_ref, $sample_index_ref) = @_;
	my $svg_file = $output;
	my $svg_pdf = 1;
	if($svg_file =~ /\.pdf$/i){ $svg_file =~ s/\.pdf$/.svg/i; $svg_pdf = 2;}
...
sub draw_density_chr_block {
	my ($SVG1, $contig_len_ref, $sample_color_ref, $SNP_density_ref, $sample_index_ref, $track_ref, $smooth_ref) = @_;
	my $chr_number = 0;
	...
	foreach my $CHR (sort keys %{$contig_len_ref})
	{
		...
		if($smooth_ref==1)
		{
			foreach my $CHR (sort keys %hash_density_color)
			{
				my @bins;
				foreach my $start (sort { $a <=> $b } keys %{ $hash_density_color{$CHR} }) {
					foreach my $end (sort { $a <=> $b } keys %{ $hash_density_color{$CHR}{$start} }) {
				...
				foreach my $bin (@bins) {
					my ($start, $end) = @$bin;
					my $cur_color = $hash_density_color{$CHR}{$start}{$end};

					if ($cur_color eq 'NA') {
						push @na_buffer, [$start, $end];
						next;
					}
					# 当前是非 NA
					if (defined $last_color && $last_color eq $cur_color && @na_buffer) {
						# 平滑：回填 NA
						foreach my $na_bin (@na_buffer) {
							my ($s, $e) = @$na_bin;
							$hash_density_color{$CHR}{$s}{$e} = $cur_color;
							$hash_ratio_density{$cur_color}++;
						}
					}
					# 重置状态
					@na_buffer  = ();
	...
	if((defined $SNP_density_ref))
	{
		...
		foreach my $i (sort{$a<=>$b} keys %{$sample_index_ref})
		{
			...
			if($j != $count_all)
			{
				...
				print"$sample\[$color\]\t$out_density\n";
			}
		}
	}
}

sub draw_identity_chr_block {
	my ($SVG1, $contig_len_ref, $sample_color_ref, $SNP_identity_ref, $sample_index_ref, $track_ref, $smooth_ref) = @_;
	my $chr_number = 0;
	my $max_identity = 0;
	...
	foreach my $CHR (sort keys %{$contig_len_ref})
	{
		...
		if($smooth_ref==1)
		{
			foreach my $CHR (sort keys %hash_identity_color)
			{
				my @bins;
				foreach my $start (sort { $a <=> $b } keys %{ $hash_identity_color{$CHR} }) {
					foreach my $end (sort { $a <=> $b } keys %{ $hash_identity_color{$CHR}{$start} }) {
						push @bins, [$start, $end];
					}
				}
				my $last_color;
				...
				foreach my $bin (@bins) {
					my ($start, $end) = @$bin;
					my $cur_color = $hash_identity_color{$CHR}{$start}{$end};
					...
					if (defined $last_color && $last_color eq $cur_color && @na_buffer) {
						...
						foreach my $na_bin (@na_buffer) {
							my ($s, $e) = @$na_bin;
							$hash_identity_color{$CHR}{$s}{$e} = $cur_color;
							$hash_ratio_identity{$cur_color}++;
						}
					}
					# 重置状态
					@na_buffer  = ();
		...
		if((defined $SNP_identity_ref))
		{
			...
			foreach my $start (sort{$a<=>$b} keys %{$hash_identity_color{$CHR}})
			{
				foreach my $end (sort{$a<=>$b} keys %{$hash_identity_color{$CHR}->{$start}})
				{
					my $sample = $hash_identity_color{$CHR}->{$start}->{$end};
					if (!defined $prev_sample)
					{
						$merged_start = $start;
						$merged_end   = $end;
	...
	if((defined $SNP_identity_ref))
	{
		...
		foreach my $i (sort{$a<=>$b} keys %{$sample_index_ref})
		{
			...
			if($j != $count_all)
			{
				...
				print"$sample\[$color\]\t$out_identity\n";
			}
		}
	}
}
 ...
 *************************************************************************************
  ...
  6) Generate a file information table based on the given path and file type, for use in downstream alignment or analysis:
     GenomeSyn2 --type fa --path ./your_path/ --out chr_length.info.tsv
     GenomeSyn2 --type syn --path ./your_path/ --out synteny.info.tsv
  ...
  --type           Type of input data: <fa|syn|anno>
                   fa    - genome sequences or chromosome length (FASTA/bed)
                   syn   - synteny files (tsv/bed)
                   anno  - genome annotation files (GFF/GTF/bed)
  --path     <dir> Generate a file named 'X.tsv' in the current directory(--out X.tsv), containing absolute 
                   paths of all files in the given path
  --out            Set the output file name to be generated
...
sub drawing_region {
	...
	if (exists $config_ref->{'show_region'}) {
		...
		$show_region_list = defined $config_ref->{'show_region'}->{'region_list'} ? $config_ref->{'show_region'}->{'region_list'} : 'NA';
	...
	#print"20250911:test\n\$GnumX:$GnumX\n";
	if($show_region_list ne 'NA')
	{
		open my $FL_region_list, "<", $show_region_list or die "Cannot open $show_region_list: $!";
		while(<$FL_region_list>)
		{
			chomp;
			next if /^#/;  # 跳过以 # 开头的注释行
			next if /^\s*$/;  # 跳过空行
			...
			my $GnumX = $tem[0];
			my @arrX = split(':',$tem[1]);
			my ($GnumX9, $GnameX9, $ChrX9, $StartX9, $EndX9);
			$GnameX9 = $arrX[0];
			$ChrX9 = $arrX[1];
			my $start_end = $arrX[2];
			$start_end =~ s/[,\s]//g;           # 去掉所有空格
			($StartX9, $EndX9) = split(/-/, $start_end);
			$hash_region{$GnumX} = "$GnameX9" . '_:_' . "$ChrX9" . '_:_' . "$StartX9" . '_:_' . "$EndX9";
		}
		close $FL_region_list;
		...
		foreach my $Syn_num(sort{$a<=>$b} keys %{$hash_syninfo_ref})
		{
			...
			my ($Gnum_RX, $Chr_RX, $Start_RX, $End_RX) = split('_:_',$hash_region{$ref_genome});
			my ($Gnum_QX, $Chr_QX, $Start_QX, $End_QX) = split('_:_',$hash_region{$query_genome});
			...
			while(<$FL_align>)
			{
				...
				my $chrR = $tem[0];
				my $chrQ = $tem[3];
				#print"$chrR:$Chr_RX\t$chrQ:$Chr_QX\n";
				if(($chrR eq $Chr_RX)and($chrQ eq $Chr_QX))
				{
					...
					$endQ   = $tem[5];
					my $strand = $tem[6];
					($Start_RX, $End_RX) = ($End_RX, $Start_RX) if $Start_RX > $End_RX;
					($startR,  $endR)    = ($endR,  $startR)  if $startR  > $endR;
					if (($Start_RX <= $endR && $startR <= $End_RX)and($Start_QX <= $endQ && $startQ <= $End_QX))
					{
						my $inter_startR = $Start_RX >= $startR ? $Start_RX : $startR;
						my $inter_endR   = $End_RX   <= $endR   ? $End_RX   : $endR;
						my $inter_startQ = $Start_QX >= $startQ ? $Start_QX : $startQ;
						my $inter_endQ   = $End_QX   <= $endQ   ? $End_QX   : $endQ;
						my $new_keyR = "$ref_genome" . ':' . "$chrR" . ':' . "$inter_startR" . ':' . "$inter_endR";
						my $new_keyQ = "$query_genome" . ':' . "$chrQ" . ':' . "$inter_startQ" . ':' . "$inter_endQ";
						$result_SYN{$Syn_num}->{$new_keyR}->{$new_keyQ} = $strand;
						#print"$new_keyR\t$new_keyQ\n";
					}
				}
			}
			close $FL_align;
	...
	foreach my $Syn_num(sort{$a<=>$b} keys %{$hash_syninfo_ref})
	{
		my $ref_genome = $Syn_num;
		my $query_genome = $Syn_num + 1;
		my $GnameR = $hash_Ginfo{$ref_genome}->{'name'};
		my $GnameQ = $hash_Ginfo{$query_genome}->{'name'};
		next if exists $result_SYN{$Syn_num};

		my $Syn_align = $hash_syninfo_ref->{$Syn_num}->{'align'};
		warn "[$Syn_num] No syntenic blocks were found for the input region in $Syn_align\n";
		
		unless (exists $hash_region{$query_genome})
		{
			warn "The input region was not found in the genome $query_genome:$GnameQ.\n";
			exit 0;
		}
	}
	 # my ($GnumX, $GnameX, $ChrX, $StartX, $EndX);
	 ...
	 #if(0 == 1)
	 {
		...
		#绘制染色体块及刻度线
		draw_chr_region($SVG1,\%hash_Ginfo,\%hash_region,$genome_num, $hash_gene_height_ref, $config_ref);
		if (exists $config_ref->{'show_region'}->{'gene_list'})
		{
...
#绘制给定染色体区域块
sub draw_chr_region {
	my ($SVG1, $hash_Ginfo_ref, $hash_region_ref, $genome_num_ref, $hash_gene_height_ref, $config_ref) = @_;
	#记录颜色
	my $gene_height_merge = 0;
	foreach my $Gnum (sort { $a <=> $b } keys %{$hash_Ginfo_ref})
	{
		my $Gname = $hash_Ginfo_ref->{$Gnum}->{'name'};
		my $chr_tags = $hash_Ginfo_ref->{$Gnum}->{'tags'} // "height:5;opacity:0.8;color:'#FFFFFF';"; #lw:1.5;color:#FF0000;opacity:1
		my $GH = $hash_gene_height_ref->{$Gnum};
		if (defined $chr_tags) 
		{
			if ($chr_tags =~ /height:\s*['"]?([\d.]+)['"]?/){$ChrHight = $1;}
		...
		if(defined $config_ref->{'show_region'}->{'bg_color'})
		{
			$Chrcolor = $config_ref->{'show_region'}->{'bg_color'};
		}
		my $hash_regionX = $hash_region_ref->{$Gnum};
		my ($GnameX, $ChrX, $StartX, $EndX) = split(/_:_/,$hash_regionX);
		my $chr_len = abs($EndX - $StartX) + 1;
...
sub drawing_SVG1 {
	...
	# 在左侧添加端粒的绘制空间
	my $tel_width = 0;
	if (exists $config_ref->{'telomere_info'}) {$SVG_x += (5 / 2);$tel_width = (5 / 2);}
	my $all_chr_height = 0;
	foreach my $Gnum (sort { $a <=> $b } keys %hash_Ginfo)
	{
		my $Gname = $hash_Ginfo{$Gnum}->{'name'};
		my $chr_tags = $hash_Ginfo{$Gnum}->{'tags'} // "height:5;opacity:0.8;color:'#FFFFFF';";
		my $hashG = $hash_read_chr_order{$Gnum};
		my $chr_num_c = 0;
		my $chrH_MAX = 0;
		foreach my $numX(sort{$a<=>$b} keys %{$hashG}){$chr_num_c++;}
		if (defined $chr_tags) 
		{
			if ($chr_tags =~ /height:\s*['"]?([\d.]+)['"]?/)
			{
				my $chrH = $1;
				$chrH_MAX = $chrH_MAX > $chrH ? $chrH_MAX:$chrH;
				$all_chr_height += $1*$chr_num_c;
			}
		}
		if($chrH_MAX != 0){$ChrHight = $chrH_MAX;}
	}
	
	if(exists $config_ref->{'genome_info'}->{'chromosome_height'})
	{
		$ChrHight = $config_ref->{'genome_info'}->{'chromosome_height'};
		$all_chr_height = $chr_num_max * $genome_num * $ChrHight;
	}
	if(exists $config_ref->{'synteny_info'}->{'synteny_height'})
	{
		$synteny_height = $config_ref->{'synteny_info'}->{'synteny_height'};
	}
	## 在上侧添加图注的绘制空间
	my $legend_num = 2;
	...
	my $SVG1_canvas_h = sprintf("%.0f", $all_chr_height + ($Chrtop + $Chrdottom)* $chr_num_max * $genome_num + $synteny_height * ($chr_num_max) * $genome_num + $SVG_x_top + $SVG_legend_height_merge);
	$SVG1= SVG -> new(width => $SVG1_canvas_w . 'mm', height   => $SVG1_canvas_h . 'mm', viewBox => "0 0 $SVG1_canvas_w $SVG1_canvas_h");

	#绘制共线性块
	draw_syn_block($SVG1,\%hash_syninfo,\%hash_Ginfo,$syntent_type,$genome_num);
	#绘制染色体块及刻度线
	draw_chr_block($SVG1,\%hash_Ginfo,$genome_num, $tel_width, $config_ref);
...
sub draw_chr_block {
	my ($SVG1, $hash_Ginfo_ref, $genome_num_ref, $tel_width, $config_ref) = @_;
	foreach my $Gnum (sort { $a <=> $b } keys %{$hash_Ginfo_ref})
	{
		my $Gname = $hash_Ginfo_ref->{$Gnum}->{'name'};
...
sub find_best_synteny_matches {
	...
	# 逐对相邻基因组读取比对结果，按染色体对统计比对覆盖长度，并据此累积正向/反向比对信号，用于判定染色体的最佳对应关系及是否需要翻转方向
    for my $Syn_num (sort { $a <=> $b } keys %{$hash_syninfo_ref})
	{
        ...
        while (<$FL_align>)
		{
			...
			my $Rnum = 0;
			my $Qnum = 0;
			foreach my $numX (sort{$a<=>$b} keys %{ $hash_read_chr_order{$Gnum_R} })
			{
				my $chrX = $hash_read_chr_order{$Gnum_R}{$numX};
				if($rChr eq $chrX){$Rnum = $numX;}
			}
			foreach my $numX (sort{$a<=>$b} keys %{ $hash_read_chr_order{$Gnum_Q} })
			{
				my $chrX = $hash_read_chr_order{$Gnum_Q}{$numX};
				if($qChr eq $chrX){$Qnum = $numX;}
			}
			if(($Rnum!=0)and($Qnum!=0))
			{
				my $lenR = abs($r2 - $r1) + 1;
	...
	# 将每一对染色体的累计比对长度，分别除以参考染色体和查询染色体的长度，得到双向的匹配比例，用于后续判定最佳同源染色体（RBH）和自动排序
	for my $Syn_num (sort { $a <=> $b } keys %match)
	{
        my $Gnum_R = $Syn_num;
		my $Gnum_Q = $Syn_num + 1;
	...
	# 为每条参考染色体选择一个唯一、双向最优的同源染色体，并同时确定其方向（正向 / 反向），用于后续的自动排序与共线性可视化
	for my $Syn_num (sort { $a <=> $b } keys %match)
	{
		my $Gnum_R = $Syn_num;
		my $Gnum_Q = $Syn_num + 1;
	...
	my $numX = 0;
	for(my $ix = 0; $ix<$chr_maxnum; $ix++)
	{
		my $r = $arr_ref_chrname[$ix] // 'None';
		$numX++;
		my $RX = $r;
		my @arr_chrline;
		my @arr_strandline;
	...
	foreach my $anno_position0(@anno_position)
	{
		...
		foreach my $Gnum (sort{$a<=>$b} keys %{$hash_Ginfo_ref})
		{
			...
			foreach my $chr_name (sort keys %{$hashG})
			{
				...
				{
					my $anno_H = 0;
					if($anno_position0 eq 'top'){$anno_H = $Chrtop;}
					elsif($anno_position0 eq 'bottom'){$anno_H = $Chrdottom;}
					elsif($anno_position0 eq 'middle'){$anno_H = $ChrHight;}
					$SVG1 -> rect(
						 x => ($rect_x),
						 y => ($rect_y),
						 width => $chr_width,
...
sub draw_vcf_chr_block {
	my ($SVG1, $contig_len_ref, $sample_color_ref, $SNP_density_ref, $SNP_identity_ref, $sample_index_ref, $track_ref, $smooth_ref) = @_;
	my $chr_number = 0;
	my $max_identity = 0;
	if(defined $SNP_identity_ref)
	{
	...
	foreach my $CHR (sort keys %{$contig_len_ref})
	{
		...
		if((defined $SNP_identity_ref)and(defined $SNP_density_ref))
		{
			...
			if($smooth_ref==1)
			{
				foreach my $CHR (sort keys %hash_priority_color)
				{
					my @bins;
					...
					foreach my $bin (@bins) {
						my ($start, $end) = @$bin;
						my $cur_color = $hash_priority_color{$CHR}{$start}{$end};
						...
						if (defined $last_color && $last_color eq $cur_color && @na_buffer) {
							...
							foreach my $na_bin (@na_buffer) {
								my ($s, $e) = @$na_bin;
								$hash_priority_color{$CHR}{$s}{$e} = $cur_color;
								$hash_ratio_merge{$cur_color}++;
							}
						}
						# 重置状态
						@na_buffer  = ();
...
Type of input data: <fa|syn|anno>
